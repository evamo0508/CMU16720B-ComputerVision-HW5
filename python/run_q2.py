...
y[np.arange(y_idx.shape[0]),y_idx] = 1 #40x4
...
# Q 2.2.2
# implement softmax
probs = forward(h1,params,'output',softmax) #40x4
    ...
    if 'W' in k:
        for i in range(v.shape[0]):
            for j in range(v.shape[1]):
                ...
                loss_pos, _ = compute_loss_and_acc(y, probs)

                v[i, j] = v_ij - eps
                h1 = forward(x, params, 'layer1')
                probs = forward(h1, params, 'output', softmax)
                loss_neg, _ = compute_loss_and_acc(y, probs)
